...
print("Helios Aircraft Exports: " .. gAircraft .." \n")
...
--      SendData("2098", string.format("%s",li))
        ...
        SendData("2092", string.format("%2s",check(li.UFC_ScratchPadString1Display)):gsub("_","-"):gsub("`","1"):gsub("~","2"))
        SendData("2093", string.format("%2s",check(li.UFC_ScratchPadString2Display)):gsub("_","-"):gsub("`","1"):gsub("~","2"))
        SendData("2094", string.format("%7s",check(li.UFC_ScratchPadNumberDisplay)))
        SendData("2095", string.format("%2s",check(li.UFC_Comm1Display)):gsub("`","1"):gsub("~","2"):gsub(" ",""))
        SendData("2096", string.format("%2s",check(li.UFC_Comm2Display)):gsub("`","1"):gsub("~","2"):gsub(" ",""))
	end
...
if scriptDebug > 0 then WriteToHeliosLog(thisScript,"LuaExportStart() invoked.") end
...
LuaExportBeforeNextFrame= function()
	if scriptDebug > 0 then WriteToHeliosLog(thisScript,"LuaExportBeforeNextFrame() invoked.") end
	ProcessInput()
...
LuaExportAfterNextFrame= function()
	if scriptDebug > 0 then WriteToHeliosLog(thisScript,"LuaExportAfterNextFrame() invoked.") end
...
LuaExportStop= function()
	if scriptDebug > 0 then WriteToHeliosLog(thisScript,"LuaExportStop() invoked.") end
...
LuaExportActivityNextEvent= function(t)
	if scriptDebug > 0 then WriteToHeliosLog(thisScript,"LuaExportActivityNextEvent() invoked.") end
	if scriptDebug > 0 and c == nil then WriteToHeliosLog(thisScript,"Connection object is Nil in LuaExportActivityNextEvent().") end
...
SendData = function(id, value)
    if scriptDebug > 4 then WriteToHeliosLog(thisScript,"Pre SendData: " .. id .. "=" .. value) end
	...
	if lLastData[id] == nil or lLastData[id] ~= value then
        ...
        if scriptDebug > 4 then WriteToHeliosLog(thisScript,"SendData: " .. data) end
...
ProcessInput = function()
    ...
    if lInput then
		...
		if (lCommand == "C") then
            if scriptDebug > 3 then WriteToHeliosLog(thisScript,"Command Received - " .. lInput) end
...
check = function(s)
    if type(s) == "string" then 
        --WriteToHeliosLog(thisScript,"Check: String value" .. s)
